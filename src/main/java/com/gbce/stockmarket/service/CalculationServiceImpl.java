...
import com.gbce.stockmarket.exception.UnknownGBCEAllShareIndex;
...
public class CalculationServiceImpl implements CalculationService {
    ...
    @Override
    public double getGBCEAllShareIndex() throws UnknownGBCEAllShareIndex {
        Collection<Stock> stocks = stockService.getAllStocksOfExchange();
        if (stocks == null || stocks.isEmpty()) {
            throw new UnknownGBCEAllShareIndex("Index can't be calculated");
        }
        ...
        double index = Math.pow(product.doubleValue(), BigDecimal.ONE.divide(new BigDecimal(stocks.size()),FRACTION_PRECISION,ROUNDING_POLICY).doubleValue());
